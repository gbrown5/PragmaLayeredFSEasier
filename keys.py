...
def update_keyfile():
	#Overwrite keys.ini if it already exists
	keys_f = open("keys.ini", "wb+")
...
if len(sys.argv) == 1:
	...
	print ""
	print "Usage: "
	print "Place hactool, BOOT0 (can be dumped from hekate) and BCPKG2-1-Normal-Main (use HacDiskMount on your RawNand.bin) in the same folder and run"
...
BOOT0_f = open("BOOT0", "rb")
BOOT0_data = BOOT0_f.read()
BOOT0_f.close()

PKG21PART_f = open("BCPKG2-1-Normal-Main", "rb")
...
# Stage 0 : Extract package1 from BOOT0 and get keyblob_mac_key_source, master_key_source, and keyblob_key_source_xx from it
PKG11_f = open("package1.bin", "wb+")
if "PK11" not in BOOT0_data:
	quit_with_errormsg("package1.bin was not found.  Please check the integrity of your BOOT0")

print "Using BOOT0 to get keys from package1..."
...
stage0_results = subprocess.check_output([HACTOOL_PATH, "--keyset=keys.ini", "--intype=keygen", "BOOT0"])
...
PKG11_f.write(PKG11_data)
PKG11_f.close()
subprocess.call([HACTOOL_PATH, "--keyset=keys.ini", "--package1dir=package1", "--intype=pk11", "package1.bin"], stdout=FNULL)
...
subprocess.call([HACTOOL_PATH, "--keyset=keys.ini", "--package2dir=package2", "--ini1dir=ini1", "--intype=pk21", "package2.bin"], stdout=FNULL)
...
stage3_results = subprocess.check_output([HACTOOL_PATH, "--keyset=keys.ini", "--intype=keygen", "BOOT0"])
parse_hactool(stage3_results)
update_keyfile()

print "If there were no warnings, we found all the keys!"
print "Now you can do hactool --keyset=keys.ini to use them!"
exit();